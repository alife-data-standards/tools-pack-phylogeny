...
@click.command()
@click.option('--cutoff', default=500, help='Minimum population size to include.')
@click.argument('pattern')
def aggregate(pattern, cutoff):
    ...
    all_edge_data.to_csv("all_phylogeny_edges.csv", index=False)
    all_size_data.to_csv("all_phylogeny_sizes.csv", index=False)
